...
#define LSREC_ERR_LENGTH_EXTENDS_OUT_OF_BOUNDS -11
#define LSREC_ERR_INVALID_CHECKSUM -12 // checksum invalid
#define LSREC_ERR_RECORD_COUNT_NONEQUAL -13 // the count of S1, S2, and S3 records given in S5 is incongruent to recieved
#define LSREC_ERR_ENTRY_OUT_OF_BOUNDS -14 // entry point specified in S7, S8, or S9 is not defined in the address of the data
#define LSREC_ERR_DATA_GIVEN_AFTER_ENTRY -15 // S1, S2, S3 after S7, S8, or S9
#define LSREC_ERR_INVALID_FORMAT -16 // if S5 len is not 3
#define LSREC_ERR_INVALID_HEX -17

int lsrec_begin();
// line is null terminated, last char is end of checksum byte
int lsrec_in(char *line, byte len);
